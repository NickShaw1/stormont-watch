...
export async function generateMetadata({ params }: { params: Promise<{ mandate: string }> }): Promise<Metadata> {
  ...
  return {
    title: `Votes - ${m?.label ?? id} archive`,
    description: `Every recorded division in the Northern Ireland Assembly during the ${m?.label ?? id} mandate.`,
  }
}

export default async function ArchiveVotesPage({ params }: { params: Promise<{ mandate: string }> }) {
  const { mandate: id } = await params
  const mandate = mandateById(id)
  if (!mandate) notFound()
  return <VotesPageBody mandate={mandate} basePath={`/archive/${id}`} />
}
