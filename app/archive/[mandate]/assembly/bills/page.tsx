...
export async function generateMetadata({ params }: { params: Promise<{ mandate: string }> }): Promise<Metadata> {
  ...
  return {
    title: `Legislation - ${m?.label ?? id} archive`,
    description: `Bills and acts in the Northern Ireland Assembly during the ${m?.label ?? id} mandate.`,
  }
}

export default async function ArchiveBillsPage({ params }: { params: Promise<{ mandate: string }> }) {
  const { mandate: id } = await params
  const mandate = mandateById(id)
  if (!mandate) notFound()
  return <BillsPageBody mandate={mandate} basePath={`/archive/${id}`} />
}
